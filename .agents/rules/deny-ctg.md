# Agent Rule: Deny `ctg` Command Execution and Secret File Access

The `ctg` and `ctgx` CLI commands are strictly forbidden from being executed by Agents (the agent) in this workspace.
...
Agents must also never view, read, edit, create, or otherwise access secret files:
