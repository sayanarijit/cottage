...
[ctg init](2_init.md) auto generates a new key for convenience. You can (but don't have to) use the same key across multiple projects.
...
If you already have an SSH key pair (e.g. the one you use with git), you can use it with cottage by adding a symlink to the private key in the `.cottage/identity` directory, and copying the public key to the `.cottage/recipients` directory.

(cott)age is compatible with RSA and Ed25519 keys that are geneated without passphrase. You can always generate a new SSH (e.g. RSA) key using `ssh-keygen` (e.g. `ssh-keygen -t rsa`) to use with cottage.
