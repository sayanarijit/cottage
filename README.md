...
- **Rich diffs**: Keeps git diff clean & review-able, while `ctg diff` shows diff of locally modified secrets with tracked encrypted counterparts.
- **Checksum verification**: Prevents tempering and makes sure that the changes in secrets and recipients are correct and intentional.
...
- **Clean up**: `ctg clean` deletes all decrypted secrets from local repo to let you run your AI agents with a tiny bit lesser worry.
...
# │   ├── identity        <- Keep it safe, or move it to `~/.ssh/`. or replace it with your existing private key. NEVER encrypt or commit it
...
ctg run ./deploy.sh  # decrypts all .cott.age files repo.
